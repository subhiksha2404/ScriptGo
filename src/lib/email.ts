...
export async function sendScriptReadyEmail(email: string, scriptTitle: string, scriptContent: any) {
    try {
        const contentHtml = Array.isArray(scriptContent)
            ? scriptContent.map((row: any) => `
          <div style="margin-bottom: 15px; padding: 10px; background-color: #f9f9f9; border-left: 4px solid #0070f3;">
            <p><strong>Visual:</strong> ${row.visual}</p>
            <p><strong>Audio:</strong> ${row.audio}</p>
          </div>
        `).join('')
            : `<p>${JSON.stringify(scriptContent)}</p>`;
